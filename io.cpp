...
static void io_redisplay_visible_monsters(dungeon *d, pair_t cursor)
{
  ...
  for (pos[dim_y] = -PC_VISUAL_RANGE;
    ...
    for (pos[dim_x] = -PC_VISUAL_RANGE;
      ...
      } else if (d->coinmap[d->PC->position[dim_y] + pos[dim_y]]
                          [d->PC->position[dim_x] + pos[dim_x]] &&
                 (can_see(d, d->PC->position,
                          d->coinmap[d->PC->position[dim_y] + pos[dim_y]]
                                   [d->PC->position[dim_x] +
                                    pos[dim_x]]->get_position(), 1, 0) ||
                 d->coinmap[d->PC->position[dim_y] + pos[dim_y]]
                          [d->PC->position[dim_x] + pos[dim_x]]->hasSeen)) {
...
void io_display(dungeon *d)
{
  ...
  for (visible_monsters = -1, pos[dim_y] = 0;
    ...
    for (pos[dim_x] = 0; pos[dim_x] < DUNGEON_X; pos[dim_x]++) {
      ...
      else if(d->coinmap[pos[dim_y]][pos[dim_x]] != NULL && (d->coinmap[pos[dim_y]]
                           [pos[dim_x]]->hasSeen || can_see(d, character_get_pos(d->PC), pos, 1, 0))){
      	attron(COLOR_PAIR(COLOR_YELLOW));
      	attron(A_BLINK);
      ...
      else if (d->objmap[pos[dim_y]][pos[dim_x]] && (d->objmap[pos[dim_y]][pos[dim_x]]->have_seen() ||can_see(d, character_get_pos(d->PC), pos, 1, 0))) {
...
static void io_redisplay_non_terrain(dungeon *d, pair_t cursor)
{
  ...
  for (pos[dim_y] = 0; pos[dim_y] < DUNGEON_Y; pos[dim_y]++) {
    for (pos[dim_x] = 0; pos[dim_x] < DUNGEON_X; pos[dim_x]++) {
      ...
      else if (d->coinmap[pos[dim_y]][pos[dim_x]] != NULL){
        attron(A_BLINK);
      	attron(COLOR_PAIR(COLOR_YELLOW));
      	mvaddch(pos[dim_y] + 1, pos[dim_x], 'O');
      	attroff(COLOR_PAIR(COLOR_YELLOW));
...
uint32_t io_print_buy(dungeon *d){
	uint32_t key;
	object *o;
	//uint32_t i;
	char s[61];
	mvprintw(5, 10, "Coin Count: %d " , d->coinCount);
	/*  for (i = 0; i < MAX_INVENTORY; i++) {
    		io_object_to_string(d->PC->in[i], s, 61);
    		mvprintw(i + 7, 10, " %c) %-55s ", '0' + i, s);
 	 }*/
 	 
 	 mvprintw(7, 10, "Press 'b' to buy a random item for 5 coins.");
 	 mvprintw(8, 10, "Press any other key to exit.");
 	 
 	 key = getch();
 	 
 	 if(key == 'b'){
 	 	if(d->PC->has_open_inventory_slot() && d->coinCount >= 5){
 	 		d->coinCount -= 5;
 	 		o = d->get_rand_object(d);
 	 		io_object_to_string(o, s, 61);
 	 		
 	 		attron(COLOR_PAIR(COLOR_GREEN));
 	 		attron(A_BOLD);
 	 		mvprintw(10, 10, "You bought %-55s." , s);
 	 	...
 	 	else if(!d->PC->has_open_inventory_slot()){
 	 		//io_queue_message("Your inventory is too full.");
 	 		attron(A_BOLD);
 	 		attron(COLOR_PAIR(COLOR_RED));
 	 		mvprintw(10,10, "Your inventory is too full.");
 	 		attroff(COLOR_PAIR(COLOR_RED));
 	 		attroff(A_BOLD);
 	 	}
 	 	
 	 	else if(d->coinCount < 5){
 	 		//io_queue_message("You do not have enough coins.");
 	 		attron(COLOR_PAIR(COLOR_RED));
 	 		attron(A_BOLD);
 	 		mvprintw(10,10, "You do not have enough coins.");
 	 		attroff(COLOR_PAIR(COLOR_RED));
 	 		attroff(A_BOLD);
 	 	}
