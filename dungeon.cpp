...
void gen_coin(dungeon *d){
/*
	for(int row = 0; row <	DUNGEON_Y; row++){
		for(int col = 0; col < DUNGEON_X; col++){
			int spawn = rand() % 100;
			
			if (spawn <= 45 && (d->map[col][row] != ter_wall || d->map[col][row] != ter_wall_immutable)){
				coin *c = new coin();
				c->setValue(1);
				d->coinmap[col][row] = c;
			}
	 ...
	 coin *c;
	  ...
	  d->coinmap[p[dim_y]][p[dim_x]] = c;
	
}

void gen_coinmap(dungeon *d){
	
	d->coinCount = 0;
	memset(d->coinmap, 0, sizeof (d->coinmap));
	d->max_coins = 35;
	
	  for (int i = 0; i < d->max_coins; i++) {
    		gen_coin(d);
  	}
...
object* dungeon::get_rand_object(dungeon *d){
	object *o;
	std::vector<object_description> &v = d->object_descriptions;
	pair_t p;

	int objInt = (rand() % d->num_objects);
	
	while(!v[objInt].can_be_generated()){
		objInt = rand() % d->num_objects;
	}
	
	p[dim_x] = 3;
	p[dim_y] = 3;
	
	o = new object(v[objInt], p, d->objmap[p[dim_y]][p[dim_x]]);
