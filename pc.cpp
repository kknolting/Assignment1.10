...
uint32_t pc::pick_up_coin(dungeon *d){
	while(d->coinmap[position[dim_y]][position[dim_x]] != NULL){
		d->coinCount++;
		d->coinmap[position[dim_y]][position[dim_x]] = NULL;
		io_queue_message("You picked up a coin.");
	}
